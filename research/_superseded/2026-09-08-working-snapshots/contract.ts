...
/** The transform returns a whole assignment inside a JSON string, and real assignments contain quotation marks
 *  ('a rating of "poor,"') and line breaks. The model escapes them almost always — but on ~1 call in 4 with a
 *  quote-heavy passage it does not, and a strict parse then throws away a good version. This walks the text,
 *  tracking string state, and escapes the two things models actually get wrong: a content quote (one not followed
 *  by a character that could continue the JSON grammar) and a raw control character inside a string. */
function repairJson(src: string): string {
  ...
  for (let i = 0; i < src.length; i++) {
    ...
    if (ch === '"') {
      ...
      const n = src[j];
      if (j >= src.length || n === "," || n === ":" || n === "}" || n === "]") { inStr = false; out += ch; }
      else out += '\\"';
      continue;
    }
    if (ch === "\n") { out += "\\n"; continue; }
...
export function extractJson(text: string): Record<string, unknown> {
  ...
  // Strict first: well-formed output is never touched. The repair runs only on output that would otherwise be lost.
